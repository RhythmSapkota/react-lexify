import { n as vt, o as Ct, s as Ot, a as Pt, b as Gt, c as St, g as C, l as B, d as At, u as It, y as Nt, q as Ht, j as D, z as Bt } from "./index-BXM0AnV6.mjs";
  ...
  x.forEach((_) => {
    ...
    if (l) {
      ...
      if (z === P.HIGHLIGHT) {
        ...
        if (M.attr("cx", g), M.attr("cy", k), M.attr("r", b.type === P.MERGE ? 9 : 10), M.attr(
          ...
          A.attr("cx", g), A.attr("cy", k), A.attr("r", 6), A.attr(
            "class",
    ...
    if (w === "TB" ? Y[b.id] = { x: g, y: f } : Y[b.id] = { x: f, y: k }, l) {
      if (b.type !== P.CHERRY_PICK && (b.customId && b.type === P.MERGE || b.type !== P.MERGE) && h.showCommitLabel) {
        ...
        if (A.attr("x", f - o.width / 2 - 2).attr("y", k + 13.5).attr("width", o.width + 2 * 2).attr("height", o.height + 2 * 2), w === "TB" && (A.attr("x", g - (o.width + 4 * 4 + 5)).attr("y", k - 12), S.attr("x", g - (o.width + 4 * 4)).attr("y", k + o.height - 12)), w !== "TB" && S.attr("x", f - o.width / 2), h.rotateCommitLabel)
          if (w === "TB")
            S.attr("transform", "rotate(-45, " + g + ", " + k + ")"), A.attr("transform", "rotate(-45, " + g + ", " + k + ")");
          else {
      ...
      if (b.tag) {
        ...
        M.attr("class", "tag-label-bkg").attr(
          ...
          ${f + o.width / 2 + 4},${d + u + 2}
          ${f - o.width / 2 - 4},${d + u + 2}`
        ), A.attr("cx", p - o.width / 2 + 4 / 2).attr("cy", d).attr("r", 1.5).attr("class", "tag-hole"), w === "TB" && (M.attr("class", "tag-label-bkg").attr(
          "points",
          `
            ${g},${p + 2}
            ${g},${p - 2}
...
//# sourceMappingURL=gitGraphDiagram-72cf32ee-tsI8d05F.mjs.map
